...
<script type="text/javascript">
    var selected_group = "";
    var selected_midterm = "{{ request()->midterm }}";
    ...
    $(document).on('change', '#group', function(e) {
        ...
        getGroup(selected_group,selected_midterm);
    });

    function getGroup(group,midterm)
    {

      return $.ajax({
            headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
            url      : "{{ url('lecturer/midterm2/getStatus') }}",
            method   : 'POST',
            data 	 : {group: group,midterm: midterm },
            error:function(err){
                alert("Error");
                console.log(err);
            },
...
$('#savebtn').click(function (e){
  ...
  formData.append('midtermid', JSON.parse(JSON.stringify(midtermid)));

  $.ajax({
        headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
        url: '{{ url('/lecturer/midterm/update') }}',
        type: 'POST',
        data: formData,
        cache : false,
        processData: false,
        ...
        success:function(res){
            try{
                if(res.message == "Success"){
                    alert("Success! midterm's Results has been updated/created!");
