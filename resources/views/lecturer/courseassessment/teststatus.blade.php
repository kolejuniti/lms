                              ...
                              <tr>
                                ...
                                @if (count($status[$key]) > 0)
                                  ...
                                  <td class="project-actions text-center" >
                                    <a class="btn btn-success btn-sm mr-2" href="/lecturer/test/{{ request()->test }}/{{ $sts->userid }}/result" {{ !empty($period) ? ($period && \Carbon\Carbon::parse(now())->format('Y-m-d') >= \Carbon\Carbon::parse($period->Start)->format('Y-m-d') && \Carbon\Carbon::parse(now())->format('Y-m-d') <= \Carbon\Carbon::parse($period->End)->format('Y-m-d') && in_array(auth()->user()->ic, json_decode($period->user_ic)) && in_array(Session::get('SessionID'), json_decode($period->session)) ? '' : 'hidden') : ''}}>
                                        <i class="ti-pencil-alt">
                                        </i>
                                        Answer
                                    </a>
                                    @if(date('Y-m-d H:i:s') >= $qz->date_from && date('Y-m-d H:i:s') <= $qz->date_to)
                                      <a class="btn btn-danger btn-sm mr-2" onclick="deleteStdTest('{{ $sts->id }}')" {{ !empty($period) ? ($period && \Carbon\Carbon::parse(now())->format('Y-m-d') >= \Carbon\Carbon::parse($period->Start)->format('Y-m-d') && \Carbon\Carbon::parse(now())->format('Y-m-d') <= \Carbon\Carbon::parse($period->End)->format('Y-m-d') && in_array(auth()->user()->ic, json_decode($period->user_ic)) && in_array(Session::get('SessionID'), json_decode($period->session)) ? '' : 'hidden') : ''}}>
                                          <i class="ti-trash">
                                          </i>
                                          Delete
                                      </a>
...
<script type="text/javascript">
    ...
    function getGroup(group)
    {
      ...
      return $.ajax({
            ...
            url      : "{{ url('lecturer/test/' . request()->id . '/' . request()->test . '/getGroup') }}",
            method   : 'POST',
            data 	 : {group: group},
            error:function(err){
                alert("Error");
