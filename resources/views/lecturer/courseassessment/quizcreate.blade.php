                            ...
                            <div class="card-body">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-9 show-published-element" style="display: none;">
                                        <button id="print-quiz" class="btn btn-success pull-right m-1"><i class="mdi mdi-printer"></i> Print Exam Paper</button>
                                    </div>
                                </div>
                            </div>
                        </div>
...
jQuery(function($) {
    ...
    if(reuse == '')
    {
        console.log('Checking quiz status. Current status:', quiz_status, 'Type:', typeof quiz_status);
        
        if(quiz_status == 2 || quiz_status === "2"){

            console.log('Quiz Status is 2 - showing read-only form');
            console.log('Quiz data available:', quiz);
            console.log('Quiz FormData:', quizFormData);
            ...
            if(quizFormData && quizFormData.length > 0) {
                ...
                setTimeout(function() {
                    renderForm(quizFormData, true);
                    $('#fb-rendered-form').show();
                    $('.header-setting *').attr('disabled', 'disabled');
                }, 300);
                
                // Show notification that quiz is published
                Swal.fire({
                    title: "Quiz is Published",
                    text: "This quiz has been published and is now in read-only mode",
                    icon: "info",
                    confirmButtonText: "OK"
                });
            } else {
                console.log('No quiz content found, showing placeholder');
                // No quiz content found
                $('#fb-rendered-form').show();
                $('#fb-render').html('<div class="alert alert-info"><h4>No Quiz Content</h4><p>This quiz has no content to display.</p></div>');
                $('.header-setting *').attr('disabled', 'disabled');
                
                Swal.fire({
                    title: "Quiz is Published",
                    text: "This quiz is published but has no content to display",
                    icon: "warning",
                    confirmButtonText: "OK"
                });
            }
...
function createQuestionWithOptions(questionType, customOptions, correctAnswers, questionParagraph, questionMark, imageFile) {
    ...
    // Print Quiz Event Handler
    $(document).on('click', '#print-quiz', function(e){
        e.preventDefault();
        printExamPaper();
    });
...
function renderForm(formdata, readonly = false){
    ...
    if (!originalFormData || originalFormData.length === 0) {
        ...
        $(fbRender).html('<div class="alert alert-warning"><h4>No Content</h4><p>No quiz content available to display.</p></div>');
        return;
    }
    ...
    try {
        ...
        if(readonly) {
            setTimeout(function() {
                ...
                $('#fb-rendered-form').addClass('quiz-readonly');
...
function renderFormManually(formdata, readonly = false) {
    ...
    if (!formdata || formdata.length === 0) {
        html += '<div class="alert alert-info"><h4>No Quiz Questions</h4><p>This quiz has no questions to display.</p></div>';
    } else {
        html += '<h4>Quiz Content (Read-Only)</h4>';
...
function printExamPaper() {
    ...
    if (!quizFormData || quizFormData.length === 0) {
        alert('No quiz content available to print.');
        return;
    }

    // Create exam paper HTML
    const examPaperHtml = generateExamPaperHTML(quizFormData);
    ...
    printWindow.document.write(`
        ...
        <html>
        <head>
            <title>Exam Paper - ${quiz.title || 'Quiz'}</title>
            <style>
                ${getExamPaperStyles()}
            </style>
        </head>
...
function generateExamPaperHTML(formData) {
    ...
    html += `
        <div class="exam-header">
            <h1 class="exam-title">${quiz.title || 'Quiz Examination'}</h1>
            <div class="exam-info">Course: ${quiz.course_name || 'Course Name'}</div>
            <div class="exam-info">Date: ${new Date().toLocaleDateString()}</div>
            <div class="exam-info">Duration: ${quiz.duration || 'Not specified'} minutes</div>
            <div class="exam-info">Total Marks: ${quiz.total_mark || 'Not specified'}</div>
        </div>
    `;
