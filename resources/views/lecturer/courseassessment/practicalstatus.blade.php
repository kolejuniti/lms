                              ...
                              <tr>
                                ...
                                @if (count($status[$key]) > 0)
                                  ...
                                  <td class="project-actions text-center" >
                                    <a class="btn btn-success btn-sm mr-2" href="/lecturer/practical/{{ request()->practical }}/{{ $sts->userid }}/result" {{ !empty($period) ? ($period && \Carbon\Carbon::parse(now())->format('Y-m-d') >= \Carbon\Carbon::parse($period->Start)->format('Y-m-d') && \Carbon\Carbon::parse(now())->format('Y-m-d') <= \Carbon\Carbon::parse($period->End)->format('Y-m-d') && in_array(auth()->user()->ic, json_decode($period->user_ic)) && in_array(Session::get('SessionID'), json_decode($period->session)) ? '' : 'hidden') : ''}}>
                                        <i class="ti-user">
                                        </i>
                                        Students
                                    </a>
