...
<script type="text/javascript">
    ...
    $(document).on('change', '#group', function(e) {
        ...
        getGroup(selected_group,selected_extra);
    });

    function getGroup(group,extra)
    {

      return $.ajax({
            headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
            url      : "{{ url('lecturer/extra/getStatus') }}",
            method   : 'POST',
            data 	 : {group: group,extra: extra },
            error:function(err){
                alert("Error");
                console.log(err);
            },
...
$('#savebtn').click(function (e){
  ...
  formData.append('extraid', JSON.parse(JSON.stringify(extraid)));

  $.ajax({
        headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
        url: '{{ url('/lecturer/extra/update') }}',
        type: 'POST',
        data: formData,
        cache : false,
        processData: false,
