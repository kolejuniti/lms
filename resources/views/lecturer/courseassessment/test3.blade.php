                          ...
                          <tbody>
                            ...
                            <tr>
                              ...
                              <td class="project-actions text-right" >
                                <a class="btn btn-success btn-sm mr-2" href="/lecturer/test3/{{ Session::get('CourseIDS') }}/{{ $dt->id }}">
                                    <i class="ti-user">
                                    </i>
                                    Students
                                </a>
                                <a class="btn btn-info btn-sm btn-sm mr-2" href="/lecturer/test3/{{ Session::get('CourseID') }}/create?testid={{ $dt->id }}">
...
$(document).ready( function () {
    ...
    $(document).on('click', '#newFolder', function() {
        location.href = "/lecturer/test3/{{ Session::get('CourseID') }}/create";
    })
  ...
  function getExtend(id)
  {
    ...
    return $.ajax({
            headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
            url      : "{{ url('lecturer/test3/getextend') }}",
            method   : 'POST',
            data 	 : {id: id},
            error:function(err){
                alert("Error");
