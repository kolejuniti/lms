...
<script type="text/javascript">
    ...
    function getGroup(group)
    {
      ...
      return $.ajax({
            ...
            url      : "{{ url('lecturer/test4/' . request()->id . '/' . request()->test . '/getGroup') }}",
            method   : 'POST',
            data 	 : {group: group},
            error:function(err){
                alert("Error");
...
$('#savebtn').click(function (e){
  ...
  formData.append('testid', JSON.parse(JSON.stringify(testid)));

  $.ajax({
        headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
        url: '{{ url('/lecturer/test4/update') }}',
        type: 'POST',
        data: formData,
        cache : false,
        processData: false,
        ...
        success:function(res){
            try{
                if(res.message == "Success"){
                    alert("Success! test's Results has been updated/created!");
