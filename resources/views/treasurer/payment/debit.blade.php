@extends((Auth::user()->usrtype == "TS") ? '../layouts.treasurer' : (Auth::user()->usrtype == "FN" ? '../layouts.finance' : (Auth::user()->usrtype == "AR" ? '../layouts.pendaftar_akademik' : (Auth::user()->usrtype == "HEA" ? '../layouts.hea' : (Auth::user()->usrtype == "HEP" ? '../layouts.hep' : (Auth::user()->usrtype == "OTR" ? '../layouts.other_user' : ''))))))
          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Debit Note</h3>
              </div>
              <!-- /.card-header -->
              <div class="card mb-3">
                <div class="card-header">
...
<script type="text/javascript">
  ...
  function getStudInfo(student) {
    return $.ajax({
      ...
      url: "{{ url('/treasurer/payment/debit/getStudent') }}",
      method: 'POST',
      data: {
        student: student
      },
  ...
  function save(ic) {
    ...
    $.ajax({
      ...
      url: "{{ url('/treasurer/payment/debit/storeDebit') }}",
      type: 'POST',
      data: formData,
      cache: false,
      processData: false,
      ...
      success: function(res) {
        try {
          if (res.message == "Success") {
            alert("Success! Payment Debit has been added!");
            location.reload();
          } else {
            $('.error-field').html('');
