                                ...
                                <td class="project-actions text-center" >
                                  @if (empty($sts) || ($sts->status == 1) )
                                    @if ($qz->status == 2)
                                      ...
                                      <a class="btn btn-success btn-sm mr-2" href="/student/test3/{{ Session::get('CourseIDS') }}/{{ request()->test }}/view">
                                        <i class="ti-user">
                                        </i>
                                        Answer
                                      </a>
                                      @elseif (date("Y-m-d H:i:s") < $qz->date_from)
                                        <a class="btn btn-danger btn-sm mr-2">
                                          ...
                                          Test will be open on {{  $qz->date_from  }}
                                        </a>
                                      @elseif (date("Y-m-d H:i:s") > $qz->date_to)
                                        <a class="btn btn-danger btn-sm mr-2">
                                          <i class="ti-lock">
                                          </i>
                                          Test has closed on {{  $qz->date_to  }}
                                        </a>
                                      @endif
                                    @elseif ($qz->status == 1)
                                      <a class="btn btn-danger btn-sm mr-2">
                                    ...
                                    @endif
                                  @elseif (!empty($sts))
                                  <a class="btn btn-success btn-sm mr-2" href="/student/test3/{{ request()->test }}/{{ Session::get('StudInfos')->ic }}/result">
                                    <i class="ti-user">
                                    </i>
                                    Result
                                  </a>
...
$(document).ready( function () {
    ...
    $(document).on('click', '#newFolder', function() {
        location.href = "/lecturer/test/{{ Session::get('CourseID') }}/create";
    })
