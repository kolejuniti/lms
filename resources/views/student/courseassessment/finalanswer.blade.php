    ...
    <div class="container-full">
        ...
        <div class="content-header">
            <div class="d-flex align-items-center">
                <div class="me-auto">
                    <h4 class="page-title">{{ $data['finaltitle'] }}</h4>
                    <div class="d-inline-block align-items-center">
                        <nav>
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">{{ $data['finaltitle'] }}</li>
                            </ol>
                        </nav>
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    <div class=" d-flex justify-content-center">
                        <div class="box text-center">
                            ...
                            <div class="box-body">
                                <h1 class="countnm fs-40 m-0" id="final-timer">
                                    {{ sprintf("%02d:%02d:00",   0, 0) }}
                                </h1>
                            </div>
                            <div class="box-body">
                                <button id="start-final-btn" onclick="startFinal()" class="waves-effect waves-light btn btn-lg btn-primary-light"><i class="fa fa-play"></i> Start Final</button>
                            </div>
                            <div class="box-body">
                                <a class="btn btn-info btn-sm mr-2">
                                    <i class="ti-alert">
                                    </i>
                                    The Final will be auto submit on {{ $data['finalendduration'] }}
                                </a>
                            </div>
                        </div>
                    </div>
...
var selected_final = "{{ $data['finalid'] }}";
var finalduration = parseInt("{{ $data['finalduration'] }}");
var finalstarttime = parseInt("{{ $data['finalstarttime'] }}");
var finaltimeleft = "{{ $data['finaltimeleft'] }}";
var finalend = "{{ $data['finalendduration'] }}";
...
function startFinal(){
    ...
    if(finaltimeleft.length > 0){
        countDownDates.setSeconds( finaltimeleft );
    }else{
        countDownDates.setSeconds( finalduration * 60 );
    }   
    ...
    var x = setInterval(function() {
        ...
        document.getElementById("final-timer").innerHTML =  hours + "h "
        + minutes + "m " + seconds + "s ";
        ...
        if (end >= d || distance < 0) {
            clearInterval(x);
            document.getElementById("final-timer").innerHTML = "TIME EXPIRED";
    ...
    $.ajax({
        ...
        url: "{{ url('student/final/startfinal') }}",
        type: 'POST',
        data:  {
...
jQuery(function($) {
    ...
    document.getElementById('save-btn').addEventListener('click', function() {
        ...
        $.ajax({
            headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
            url: "{{ url('student/final/savefinal') }}",
            type: 'POST',
            data:  {
                final: selected_final,
    ...
    document.getElementById('submit-btn').addEventListener('click', function() {
        ...
        $.ajax({
            headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
            url: "{{ url('student/final/submitfinal') }}",
            type: 'POST',
            data:  {
                id: selected_final,
                data: window.JSON.stringify( {formData: $(fbRender).formRender("userData") })
            ...
            success:function(res){
                if(!res.status){
                    alert(res.message);
                    location.href = "/student/final/{{ Session::get('CourseIDS') }}/"+ selected_final;
                }else{
                    location.href = "/student/final/{{ Session::get('CourseIDS') }}/"+ selected_final;
                }
            }
