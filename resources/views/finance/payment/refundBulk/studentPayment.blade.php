          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Student Bulk Refund Payment</h3>
              </div>
              <!-- /.card-header -->
              <div class="card mb-3">
                <div class="card-header">
                  ...
                  <div class="row">
                    <div class="col-md-6">
                      <div class="form-group">
                        <p>Bulk Refund Batch Code &nbsp; &nbsp;&nbsp;&nbsp;: &nbsp;&nbsp; BULK-{{ str_pad($payment->id, 5, '0', STR_PAD_LEFT) }}</p>
                      </div>
                    </div>
                    <div class="col-md-6">
                      <div class="form-group">
                        <p>Total Amount &nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;: &nbsp;&nbsp; RM {{ number_format($payment->amount, 2) }}</p>
                      </div>
                    </div>
                  </div>
                  <div class="row">
...
function getStudInfo(student)
{
    return $.ajax({
            headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
            url      : "{{ url('finance/payment/refund/bulk/student/getStudent') }}",
            method   : 'POST',
...
function save(ic)
{
  ...
  $.ajax({
        headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
        url: '{{ url('/finance/payment/refund/bulk/student/storeStudent') }}',
        type: 'POST',
        data: formData,
        cache : false,
        processData: false,
...
function confirm()
{
  ...
  if(sum != '' && sum2 != '')
  {
    ...
    $.ajax({
          headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
          url: '{{ url('/finance/payment/refund/bulk/student/confirmStudent') }}',
          type: 'POST',
          data: formData,
          cache : false,
          processData: false,
          ...
          success:function(res){
              try{
                  if(res.message == "Success"){
                      alert("Success! Payment Details has been added!");
                      window.open('/finance/payment/refund/bulk/getReceipt?id=' + res.id, '_blank');
