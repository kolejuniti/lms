          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Allowance Payment</h3>
              </div>
              <!-- /.card-header -->
              <div class="card mb-3">
                <div class="card-header">
...
function add()
{
  ...
  $.ajax({
        headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
        url: '{{ url('/finance/payment/allowance/payment/input/store') }}',
        type: 'POST',
        data: formData,
        cache : false,
        processData: false,
...
function add2(id)
{
  ...
  $.ajax({
        headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
        url: '{{ url('/finance/payment/allowance/payment/input/store2') }}',
        type: 'POST',
        data: formData,
        cache : false,
        processData: false,
        ...
        success:function(res){
                try{
                  if(res.message == "Success"){
                      alert("Success! Allowance Details has been added!");
...
function deletedtl(dtl,id)
{
  ...
  Swal.fire({
    ...
    if (res.isConfirmed){
              $.ajax({
                  headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
                  url      : "{{ url('finance/payment/allowance/payment/input/delete') }}",
                  method   : 'POST',
                  data 	 : {dtl:dtl, id: id},
                  error:function(err){
                      alert("Error");
...
function confirm()
{
  ...
  if(id != '')
  {
    ...
    $.ajax({
        headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
        url      : "{{ url('finance/payment/allowance/payment/input/confirm') }}",
        method   : 'POST',
        data 	 : {id: id},
        error:function(err){
            alert("Error");
        ...
        success  : function(data){
          alert("Success! Sponsorship Payment has been confirmed!");
            window.location.href = "/finance/payment/allowance/payment";
