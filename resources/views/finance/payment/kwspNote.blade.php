...
<div class="content-wrapper" style="min-height: 695.8px;">
  ...
  <div class="content-header">
    <div class="d-flex align-items-center">
      <div class="me-auto">
        <h4 class="page-title">KWSP Refund / Discount</h4>
        <div class="d-inline-block align-items-center">
          <nav>
            <ol class="breadcrumb">
              <li class="breadcrumb-item"><a href="#"><i class="mdi mdi-home-outline"></i></a></li>
              <li class="breadcrumb-item" aria-current="page">Dashboard</li>
              <li class="breadcrumb-item active" aria-current="page">KWSP Refund / Discount</li>
            </ol>
          </nav>
        </div>
      </div>
          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Student KWSP Refund / Discount</h3>
              </div>
              <!-- /.card-header -->
              <div class="card mb-3">
                <div class="card-header">
...
function getStudInfo(student)
{
    return $.ajax({
            headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
            url      : "{{ url('finance/payment/KWSPrefund/getStudent') }}",
            method   : 'POST',
...
function add(ic)
{
  ...
  formData.append('kwspDetails', JSON.stringify(forminput));

  $.ajax({
        headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
        url: '{{ url('/finance/payment/KWSPrefund/storeKWSPrefund') }}',
        type: 'POST',
        data: formData,
        cache : false,
        processData: false,
...
function deletedtl(id, ic)
{
  ...
  Swal.fire({
    ...
    if (res.isConfirmed){
              $.ajax({
                  headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
                  url      : "{{ url('finance/payment/KWSPrefund/deleteKWSPrefund') }}",
                  method   : 'POST',
                  data 	 : {id: id, ic: ic},
                  error:function(err){
                      alert("Error");
