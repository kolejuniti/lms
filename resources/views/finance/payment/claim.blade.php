          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Student Claim</h3>
              </div>
              <!-- /.card-header -->
              <div class="card mb-3">
                <div class="card-header">
...
function getStudInfo(student)
{
    return $.ajax({
            headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
            url      : "{{ url('finance/payment/claim/getStudent') }}",
            method   : 'POST',
...
function register(ic)
{

  $.ajax({
      headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
      url      : "{{ url('finance/payment/claim/registerClaim') }}",
      method   : 'POST',
      data 	 : {ic: ic},
      error:function(err){
          alert("Error");
...
function add(ic)
{
  if($('#idclaim').val() != '')
  {
    ...
    formData.append('claimDetail', JSON.stringify(forminput));

    $.ajax({
          headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
          url: '{{ url('/finance/payment/claim/addStudentClaim') }}',
          type: 'POST',
          data: formData,
          cache : false,
          processData: false,
          ...
          success:function(res){
              try{
                  if(res.message == "Success"){
                      alert("Success! Claim Details has been added!");
                      $('#claim_list').html(res.data);
                      $('#claim_list').DataTable();
                  }else{
                      $('.error-field').html('');
    ...
    alert('Please register student claim details first!');
...
function deletedtl(dtl,id)
{
  ...
  Swal.fire({
    ...
    if (res.isConfirmed){
              $.ajax({
                  headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
                  url      : "{{ url('finance/payment/claim/deleteStudentClaim') }}",
                  method   : 'POST',
                  data 	 : {dtl:dtl, id:id},
                  error:function(err){
                      alert("Error");
...
function confirm()
{
  ...
  $.ajax({
      headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
      url      : "{{ url('finance/payment/claim/confirmClaim') }}",
      method   : 'POST',
      data 	 : {id: id},
      error:function(err){
          alert("Error");
