...
<div class="content-wrapper" style="min-height: 695.8px;">
  ...
  <div class="content-header">
    <div class="d-flex align-items-center">
      <div class="me-auto">
        <h4 class="page-title">Incentive Payment</h4>
...
function getStudInfo(student)
{
    return $.ajax({
            headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
            url      : "{{ url('finance/payment/incentive/getStudent') }}",
            method   : 'POST',
...
function save(ic)
{
  ...
  $.ajax({
        headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
        url: '{{ url('/finance/payment/incentive/storeIncentive2') }}',
        type: 'POST',
        data: formData,
        cache : false,
        processData: false,
...
function add(ic)
{
  if($('#idpayment').val() != '')
  {
    ...
    $.ajax({
          headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
          url: '{{ url('/finance/payment/incentive/storeIncentiveDtl') }}',
          type: 'POST',
          data: formData,
          cache : false,
          processData: false,
...
function deletedtl(dtl,id)
{
  ...
  Swal.fire({
    ...
    if (res.isConfirmed){
              $.ajax({
                  headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
                  url      : "{{ url('finance/payment/incentive/deleteIncentive') }}",
                  method   : 'POST',
                  data 	 : {dtl:dtl, id: id},
                  error:function(err){
                      alert("Error");
...
function confirm()
{
  ...
  if(sum != '' && sum2 != '')
  {
    ...
    $.ajax({
          headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
          url: '{{ url('/finance/payment/incentive/confirmIncentive') }}',
          type: 'POST',
          data: formData,
          cache : false,
          processData: false,
