          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Pre - Registration</h3>
              </div>
              <!-- /.card-header -->
              <div class="card mb-3">
                <div class="card-header">
...
function getStudInfo(student)
{
    return $.ajax({
            headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
            url      : "{{ url('finance/payment/other/getStudent') }}",
            method   : 'POST',
...
function save(ic)
{
  ...
  $.ajax({
        headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
        url: '{{ url('/finance/payment/other/storePayment') }}',
        type: 'POST',
        data: formData,
        cache : false,
        processData: false,
...
function add(ic)
{
  if($('#idpayment').val() != '')
  {
    ...
    $.ajax({
          headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
          url: '{{ url('/finance/payment/other/storePaymentDtl') }}',
          type: 'POST',
          data: formData,
          cache : false,
          processData: false,
...
function deletedtl(dtl,meth,id)
{
  ...
  Swal.fire({
    ...
    if (res.isConfirmed){
              $.ajax({
                  headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
                  url      : "{{ url('finance/payment/other/deletePayment') }}",
                  method   : 'POST',
...
function confirm()
{
  ...
  $.ajax({
      headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
      url      : "{{ url('finance/payment/other/confirmPayment') }}",
      method   : 'POST',
      data 	 : {id: id},
      error:function(err){
          alert("Error");
