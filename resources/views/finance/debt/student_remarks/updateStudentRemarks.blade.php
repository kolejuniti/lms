                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select 
                            ...
                            <option value="{{ $cat->id }}" 
                                {{ (isset($data['remarks']) && count($data['remarks']) > 0 && $data['remarks'][0]->category_id == $cat->id) ? 'selected' : '' }}>
                                {{ $cat->name }}
                            </option>
                            @endforeach
                        </select>
            ...
            <div class="row" id="stud_info">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="float" class="form-control" id="correction" name="correction" 
                            value="{{ (isset($data['remarks']) && count($data['remarks']) > 0) ? $data['remarks'][0]->correction_amount : '' }}" required>
                    </div>
                </div>          
                <div class="col-md-6">  
                    <div class="form-group">
                        <label class="form-label" for="current">Current Amount</label>
                        <input type="float" class="form-control" id="current" name="current" 
                            value="{{ (isset($data['remarks']) && count($data['remarks']) > 0) ? $data['remarks'][0]->latest_balance : '' }}" required>
                    </div>
                </div>
                <div class="col-md-12 mt-3">
                    <div class="form-group">
                        <label class="form-label">Comment</label>
                        <textarea class="form-control" id="comment" name="comment" class="mt-2" rows="10" cols="80" onkeyup="this.value = this.value.toUpperCase();" required>{{ (isset($data['remarks']) && count($data['remarks']) > 0) ? $data['remarks'][0]->notes : '' }}</textarea>
                    </div>   
                </div>
            </div>
        </div>
