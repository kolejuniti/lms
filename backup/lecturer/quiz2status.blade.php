                              ...
                              <tr>
                                ...
                                @if (count($status[$key]) > 0)
                                  ...
                                  <td class="project-actions text-center" >
                                    <a class="btn btn-success btn-sm mr-2" href="/lecturer/quiz2/{{ request()->quiz2 }}/{{ $sts->userid }}/result">
                                        <i class="ti-user">
                                        </i>
                                        Students
                                    </a>
...
<script type="text/javascript">
    var selected_group = "";
    var selected_quiz2 = "{{ request()->quiz2 }}";
    ...
    $(document).on('change', '#group', function(e) {
        ...
        getGroup(selected_group,selected_quiz2);
    });

    function getGroup(group,quiz2)
    {

      return $.ajax({
            headers: {'X-CSRF-TOKEN':  $('meta[name="csrf-token"]').attr('content')},
            url      : "{{ url('lecturer/quiz2/getStatus') }}",
            method   : 'POST',
            data 	 : {group: group,quiz2: quiz2 },
            error:function(err){
                alert("Error");
                console.log(err);
            },
