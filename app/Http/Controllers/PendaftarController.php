...
class PendaftarController extends Controller
{
    ...
    public function exportStudentEdit(Request $request)
    {
        try {
            ...
            $daerahList = \Illuminate\Support\Facades\DB::table('tbldaerah')->get();
            ...
            foreach ($students as $key => $student) {
                ...
                // Get Daerah Code
                $daerahTetap = $student->city ?? '';
                if (!empty($student->city)) {
                    $cityLower = strtolower(trim($student->city));
                    foreach ($daerahList as $daerah) {
                        $daerahNameLower = strtolower(trim($daerah->name));
                        if (strpos($daerahNameLower, $cityLower) !== false || strpos($cityLower, str_replace('daerah ', '', $daerahNameLower)) !== false) {
                            $daerahTetap = $daerah->code;
                            break;
                        }
                    }
                }
                ...
                if (isset($student->statelevel_id) && $student->statelevel_id == 1) {
                    $warganegara = '1';
                    $negara = '217';
                } else {
                    $warganegara = '2';
                ...
                $kategoriPembiayaan = '5';
                $penajaPengajian = '';
                $programPembiayaanKPT = '';
                if (!empty($student->ic)) {
                    $sponsorStudent = \Illuminate\Support\Facades\DB::table('tblpayment')
                        ->join('tblsponsor_library', 'tblpayment.payment_sponsor_id', 'tblsponsor_library.id')
                        ->where([
                        ...
                        ->select('tblsponsor_library.id')
                        ->first();

                    if ($sponsorStudent && $sponsorStudent->id == 3) {
                        $kategoriPembiayaan = '1';
                        $penajaPengajian = '8';
                        $programPembiayaanKPT = '2';
                    }
                }
                ...
                $exportData[] = [
                    ...
                    '', // Bulan Tamat Pengajian
                    '', // Tahun Tamat Pengajian
                    $kategoriPembiayaan, // Kategori Pembiayaan Penaja
                    $penajaPengajian, // Penaja Pengajian
                    '', // Keterangan Penaja Pengajian
                    $programPembiayaanKPT, // Program Pembiayaan oleh KPT
                    $spmBmId, // BM(SPM)
                    $spmBiId, // BI(SPM)
                    $student->muet_code ?? '7*', // MUET
                    '-2', // Cawangan
                    '2', // Status Francais
                    '', // Kod Institusi Francais
                    '1', // Kelayakan Masuk
    ...
    public function view()
    {
        $student = DB::table('students')
            ...
            ->select(
                ...
                'students.updated_by as student_updated_by',
        ...
        if (isset(request()->print)) {
            ...
            $student = DB::table('students')
                ...
                ->select(
                    ...
                    'tblstudent_pass.*',
                    'student_form.*',
                    'a.state_name AS place_birth',
                    'tblbatch.BatchName',
                    'tblsex.sex_name',
                    'tblnationality.nationality_name',
                    'tblreligion.religion_name',
                    'tblcitizenship_level.citizenshiplevel_name',
                    'tblcitizenship.citizenship_name',
                    'descendants.descendants_name',
                    'muet.muet_name',
                    'tblmarriage.marriage_name',
                    'tbledu_advisor.name AS advisor',
                    'tblpass_type.name AS pass_type',
                    'tblcountry.name AS country',
                    'b.state_name AS state_name2',
                    'tbldun.name AS dun',
                    'tblparlimen.name AS parlimen',
                    'tblqualification_std.name AS qualification',
                    'tblprogramme.progname',
                    'c.SessionName',
                    'd.SessionName AS session',
                    'tblstudent_status.name AS status',
                    'student_tin_no.tin_number'
                )
                ->where('students.ic', request()->ic)->first();
            ...
            $baseQuery = function () use ($request) {
                return DB::table('students')
                    ...
                    ->select(
                        'students.*',
                        'tblstudent_personal.no_tel',
                        'tblstudent_personal.qualification',
                        'sessions.SessionName',
                        'tblprogramme.progcode',
                        'tbledu_advisor.name AS ea',
                        'tblsex.code AS sex',
                        'tblstudent_status.name AS status'
                    );
            };
    ...
    public function getAnnualStudentReport(Request $request)
    {
        ...
        $data['student1'] = ($baseQuery)()  // Make sure $baseQuery is defined correctly
            ...
            ->select(
                ...
                'students.ic',
                'students.no_matric',
                'tblstudent_log.kuliah_id AS student_status',
                'tblsex.code as gender',
                'tblprogramme.progcode',
                'sessions.SessionName AS session',
                'tblstudent_log.semester_id AS semester',
                'tblstudent_log.date',
                'tblstudent_log.remark',
                'tblstudent_status.name AS status',
                'tblnationality.nationality_name AS race'
            )
            ->get();
